...
import { deleteLocalFile, persistRecording } from "@/lib/audio-storage";
...
export default function FreeFlowSpeakScreen() {
  ...
  useEffect(() => {
    if (status === "recording" && elapsedSeconds >= MAX_SECONDS) {
      void handleStop();
    }
  ...
  async function handleSave() {
    ...
    try {
      ...
      const persisted = persistRecording(previewUri, entry.id);
      ...
      {status === "requesting" ? (
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.body}>
            To record a voice reflection, enable microphone access for Graceward
        ...
        <View style={styles.card}>
          <Text style={styles.body}>
            Tap below to begin recording your reflection. Audio stays private on
            this device for now.
          </Text>
          <Button
